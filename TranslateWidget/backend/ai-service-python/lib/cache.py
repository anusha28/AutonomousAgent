...
class TwoTierCache:
    ...
    async def get(self, text: str, target: str) -> str | None:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute("SELECT translated FROM translations WHERE key = ?", (k,)) as cur:
                row = await cur.fetchone()
                if row:
                    self._stats["db_hits"] += 1
                    translated_value = row[0]
                    self._mem[k] = translated_value
                    await db.execute("UPDATE translations SET access_count = access_count + 1 WHERE key = ?", (k,))
                    await db.commit()
